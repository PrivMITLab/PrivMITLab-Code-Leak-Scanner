...
function App() {
  ...
  const [showStats, setShowStats] = useState(true);
  const [showKeyboardShortcuts, setShowKeyboardShortcuts] = useState(false);
  const [ignoreComments, setIgnoreComments] = useState(true); // NEW: toggle comment filtering
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      // Pass the ignoreComments option to the scanner
      const scanOptions = { ignoreComments };
      ...
      if (inputMode === 'paste') {
        ...
        findings = scanText(code, undefined, undefined, scanOptions);
        contentToHighlight = code;
      } else {
        if (files.length === 0) {
          showToast('Please upload or drop some files to scan', 'error');
        ...
        findings = scanFiles(files, scanOptions);
        contentToHighlight = files.map(f => f.content).join('\n');
      }
  ...
  }, [code, files, inputMode, ignoreComments, showToast, totalLines]);
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-9 h-5 bg-slate-700 rounded-full peer peer-checked:after:translate-x-full peer-checked:bg-cyan-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all"></div>
                  </label>
                  <span className="text-xs sm:text-sm font-medium">Ignore comments (// and #)</span>
                </div>
                <button
                  onClick={() => setIgnoreComments(prev => !prev)}
                  className="text-xs px-2 py-1 bg-cyan-600/20 text-cyan-400 rounded hover:bg-cyan-600/30 transition-colors"
                >
                  {ignoreComments ? 'ON' : 'OFF'}
                </button>
              </div>
              <p className={`text-[10px] sm:text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-gray-500'} mt-2`}>
                {ignoreComments 
                  ? 'Lines starting with // or # will not be scanned (reduces false positives from test keys).'
                  : 'All lines will be scanned, including comments.'}
              </p>
            </div>
