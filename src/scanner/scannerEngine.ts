...
// ----------------------------------------------------------------------
//  Highlighting helpers (with overlap merging)
// ----------------------------------------------------------------------
...
export function highlightCode(
  ...
  for (let i = 0; i < lines.length; i++) {
    const lineNum = i + 1;
    const rawLine = lines[i];
    let highlights = lineHighlights.get(lineNum) || [];
    ...
    // 2. Merge overlapping or adjacent highlights
    ...
    for (const h of highlights) {
      ...
      // If current highlight overlaps or touches the last one (h.start <= last.end)
      if (h.start <= last.end) {
        // Merge: extend end if needed, keep higher severity (high > medium > low)
        last.end = Math.max(last.end, h.end);
        const severityRank = { high: 3, medium: 2, low: 1 };
        if (severityRank[h.severity] > severityRank[last.severity]) {
